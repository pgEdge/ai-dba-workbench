...
pgEdge AI DBA Workbench. Use pre-built container images for production or
build from source for development.

!!! note
    RPM and DEB packages are available from the
    [pgEdge Enterprise Repository](https://docs.pgedge.com/enterprise/) and
    are used in the Docker deployment method. pgEdge packages create and use
    the `pgedge` user automatically; manual adjustments to systemd files are
    not required to manage the service.
...
2. Generate the required secret files for the server and the database.
   ...
   In the following example, the `openssl` command creates a random secret
   key.
...
3. Set the required PostgreSQL password as an environment variable.
   In the following example, the `export` command sets the password:

   ```bash
   export POSTGRES_PASSWORD=your-password
   ```

4. Start all services using the production Compose file.
...
5. Verify that all services are running.
   ...
   In the following example, the `ps` subcommand lists running containers
   and their status.
...
6. Open a browser and navigate to `http://localhost:3000` to access the
   web client.

7. Create an initial admin user account.

   In the following example, the `exec` subcommand runs the user creation
   command inside the server container.
...
Select a tag by setting the `VERSION` environment variable. In the following
examples, the `VERSION` variable controls which image tag the `docker compose`
command pulls.
...
containers at runtime. Edit the files in `docker/config/` to customize
the deployment.
...
The root `docker-compose.yml` file builds all images from source. Use this
...
Docker monitors automatically. Use the following commands to verify the
deployment status.

In the following example, the `ps` subcommand displays the health status
of each container.
...
Follow the logs of a specific service to diagnose issues. In the
following example, the `logs` subcommand streams the server output.
...
incorrect permissions. Check the container logs for error details.
...
The container will fail to start if another PostgreSQL instance or
other service already occupies that port. The error message includes
the text "port is already allocated."

Set the `POSTGRES_PORT` environment variable to use a different host port.
...
Verify that the database credentials in the server configuration match
the PostgreSQL password. Check the `docker/config/ai-dba-server.yaml`
file and the `docker/secret/pg-password` file for consistency.

### Viewing Logs for All Services

View logs for all services simultaneously to identify the source of a problem.
