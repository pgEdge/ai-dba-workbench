...
these tools and resources to interact with monitored databases.

## Connecting an MCP Client

The MCP server exposes a JSON-RPC 2.0 endpoint at `/mcp/v1` over HTTP or HTTPS.
...
These tools manage persistent memories that the AI assistant can store and
recall across conversations.

| Tool | Description |
|------|-------------|
| `store_memory` | Stores a persistent memory with a category, scope, and optional pinned flag. |
| `recall_memories` | Searches stored memories using semantic similarity; the search always includes pinned memories. |
...
Resources expose read-only data that MCP clients can retrieve without calling a
tool. The following resources are available:
