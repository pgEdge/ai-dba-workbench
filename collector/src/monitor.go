...
func (m *Monitor) loadProbes() error {
	...
	defer func() {
		if cerr := m.datastore.ReturnConnection(conn); cerr != nil {
...
func (mc *MonitoredConn) connect(config *Config) error {
	...
	if err := db.Ping(); err != nil {
		if cerr := db.Close(); cerr != nil {
			log.Printf("Error closing database connection after ping failure: %v", cerr)
		}
...
func (mc *MonitoredConn) buildConnectionString(_ *Config) string {
	conn := mc.connection
