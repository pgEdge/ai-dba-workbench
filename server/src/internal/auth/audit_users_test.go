...
// TestLockoutSurvivesAuditFailure drives the whole failed-login path
// with the audit log refusing every insert, and checks that the account
// is still locked afterwards. The lockout is the server defending
// itself rather than a change an operator asked for, so an audit
// failure must not roll it back and leave the attacker guessing
// against a working account.
func TestLockoutSurvivesAuditFailure(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "auth-lockout-audit-fail-*")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)
	...
	if _, err := store.db.Exec(
		`CREATE TRIGGER block_audit_insert BEFORE INSERT ON audit_events
         BEGIN SELECT RAISE(ABORT, 'audit log unavailable'); END`); err != nil {
		t.Fatalf("Failed to block audit inserts: %v", err)
	}
	...
	if locked.Enabled {
		t.Error("Expected the account to be locked even though the audit " +
			"write failed")
	}

	if got := auditEventCount(t, store); got != before {
		t.Errorf("Expected no audit event to be written, got %d new",
			got-before)
	}
}

// mustBeginTx starts a transaction and registers its rollback, for the
// tests that drive a *Tx helper directly.
func mustBeginTx(t *testing.T, s *AuthStore) *sql.Tx {
	t.Helper()
...
// TestDeleteUserRowTxMissingRow covers the race guard in
// deleteUserRowTx: the caller has already looked the user up, so a
// delete that removes nothing means the row went in between, and the
// caller must be told rather than shown a silent success.
func TestDeleteUserRowTxMissingRow(t *testing.T) {
	store, cleanup := createTestAuthStoreForAudit(t)
	defer cleanup()

	tx := mustBeginTx(t, store)

	err := deleteUserRowTx(tx, 424242, "ghost")
	if err == nil {
		t.Fatal("Expected an error when no row is deleted")
	}
	if !strings.Contains(err.Error(), "user 'ghost' not found") {
		t.Errorf("Expected a not-found error, got %v", err)
	}
}

// TestInsertUserAuditedSnapshotFailure covers the branch where the
// insert reports success but the row cannot be read back, which is what
// the after snapshot and the event's target id both depend on.
func TestInsertUserAuditedSnapshotFailure(t *testing.T) {
	store, cleanup := createTestAuthStoreForAudit(t)
	defer cleanup()

	before := auditEventCount(t, store)

	// An insert that quietly creates nothing: the snapshot read that
	// follows finds no row.
	err := store.insertUserAudited(testActor(), "user.create", "phantom",
		func(*sql.Tx) error { return nil })
	if err == nil {
		t.Fatal("Expected an error when the created user cannot be read")
	}
	if !strings.Contains(err.Error(), "failed to read created user") {
		t.Errorf("Expected a snapshot error, got %v", err)
	}

	// The mutation failed, so the log gains a failure event naming the
	// user rather than a success one.
	if got := auditEventCount(t, store); got != before+1 {
		t.Fatalf("Expected one failure event, got %d new", got-before)
	}
	ev := lastAuditEvent(t, store)
	if ev.Outcome != OutcomeFailure {
		t.Errorf("Expected a failure outcome, got %q", ev.Outcome)
	}
	if ev.TargetName != "phantom" {
		t.Errorf("Expected the event to name the user, got %q", ev.TargetName)
	}
}
