...
func buildDatabaseAnalysisPrompt(databases []DatabaseInfo) string {
	...
	b.WriteString(`Do not introduce yourself or use any greeting. Do not use `)
	b.WriteString(`casual or conversational language. Write in a professional, `)
	b.WriteString(`impersonal, third-person technical style. State facts `)
	b.WriteString(`directly without preamble.`)
	b.WriteString("\n\n")
