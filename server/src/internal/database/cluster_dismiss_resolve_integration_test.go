...
// TestResolveConnectionHierarchy_InsertBranchDoesNotResurrect covers the
// second failure mode in issue #36. If the clusters row was hard-deleted
// (or never created) but a dismissed row with the same auto_cluster_key
// already exists, the fallback INSERT must not bypass dismissal and
// create a new live row. The fix keeps the INSERT but uses
// ON CONFLICT DO NOTHING and then SELECT ... WHERE dismissed = FALSE, so
// the resolver treats the connection as unassigned rather than
// resurrecting a hidden cluster.
func TestResolveConnectionHierarchy_InsertBranchDoesNotResurrect(t *testing.T) {
	...
	// Pre-seed a dismissed clusters row for the auto key the topology
	// will produce. The initial SELECT in resolveConnectionHierarchy
	// finds this row and must return nil rather than un-dismissing it.
	var dismissedClusterID int
	if err := pool.QueryRow(ctx, `
        INSERT INTO clusters (name, auto_cluster_key, group_id, dismissed)
        VALUES ('pg17 Spock', 'spock:pg17', $1, TRUE)
        RETURNING id
    `, groupID).Scan(&dismissedClusterID); err != nil {
		t.Fatalf("seed dismissed cluster: %v", err)
	}
	...
	if cID != nil || cName != nil || gID != nil || gName != nil {
		t.Fatalf("resolver returned hierarchy for dismissed cluster: cID=%v cName=%v gID=%v gName=%v (issue #36)",
			cID, cName, gID, gName)
	}

	// The dismissed row must still be dismissed, and there must be no
	// additional live row with the same auto key.
	var stillDismissed bool
	if err := pool.QueryRow(ctx,
		`SELECT dismissed FROM clusters WHERE id = $1`, dismissedClusterID,
	).Scan(&stillDismissed); err != nil {
		t.Fatalf("read dismissed flag: %v", err)
	}
	if !stillDismissed {
		t.Fatalf("resolver un-dismissed the cluster (issue #36)")
	}
	...
	if err := pool.QueryRow(ctx, `
        SELECT COUNT(*)
        FROM clusters
        WHERE auto_cluster_key = 'spock:pg17' AND dismissed = FALSE
    `).Scan(&liveRows); err != nil {
		t.Fatalf("count live rows: %v", err)
	}
	if liveRows != 0 {
		t.Fatalf("resolver created a live clusters row shadowing a dismissed one (issue #36): liveRows=%d",
			liveRows)
	}
	...
	for _, s := range summaries {
		if s.AutoClusterKey != nil && *s.AutoClusterKey == "spock:pg17" {
			t.Fatalf("dropdown surfaced dismissed auto cluster: %+v (issue #36)", s)
		}
	}
}
