...
func (p *PgSysCPUUsageInfoProbe) Execute(ctx context.Context, connectionName string, monitoredConn *pgxpool.Conn, pgVersion int) ([]map[string]any, error) {
	...
	query := WrapQuery(ProbeNamePgSysCPUUsageInfo, p.GetQuery())
...
func (p *PgSysCPUUsageInfoProbe) Store(ctx context.Context, datastoreConn *pgxpool.Conn, connectionID int, timestamp time.Time, metrics []map[string]any) error {
	if len(metrics) == 0 {
