...
describe('TopologyPanel', () => {
    ...
    it('passes handleSourceChange to RelationshipSection', async () => {
        // This test verifies that the handleSourceChange callback is properly
        // passed to RelationshipSection. The callback behavior (resetting target
        // when source changes) is tested in RelationshipSection.test.tsx.
        const servers = [
        ...
        // Wait for the relationship section to appear (requires 2+ servers)
        await waitFor(() => {
            expect(screen.getByText('Relationships')).toBeInTheDocument();
        });

        // Verify that Source select is rendered and functional
        const sourceSelect = screen.getByLabelText('Source');
        expect(sourceSelect).toBeInTheDocument();
        expect(sourceSelect).toHaveAttribute('aria-haspopup', 'listbox');
    });

    it('passes handleRelTypeChange to RelationshipSection', async () => {
        // This test verifies that the handleRelTypeChange callback is properly
        // passed to RelationshipSection. The callback behavior (resetting target
        // when type changes) is tested in RelationshipSection.test.tsx.
        const servers = [
        ...
        // Verify that Type select is rendered and functional
