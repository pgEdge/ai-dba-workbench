...
func TestConnectionPoolCreation(t *testing.T) {
	...
	defer func() {
		if cerr := pool.Close(); cerr != nil {
			t.Errorf("Failed to close pool: %v", cerr)
		}
	}()
...
// TestConnectionPoolRequestReturn tests the connection pool with a real in-memory database
...
func TestConnectionPoolWithTestDB(t *testing.T) {
	...
	if err := testDB.Ping(); err != nil {
		if cerr := testDB.Close(); cerr != nil {
			t.Logf("Warning: failed to close test DB: %v", cerr)
		}
	...
	defer func() {
		if cerr := pool.ReturnConnection(conn3); cerr != nil {
			t.Errorf("Failed to return connection: %v", cerr)
		}
	}()
...
func TestConnectionPoolMaxConnectionsLimit(t *testing.T) {
	...
	// Try to get third connection - should timeout since pool is exhausted
	// Use a short timeout context to test the waiting behavior
	...
	if err := pool.ReturnConnection(conn3); err != nil {
		t.Errorf("Failed to return connection: %v", err)
	}
}
