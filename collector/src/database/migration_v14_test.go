...
// The v14 migration adds Telegram as a notification channel type, as
...
// telegramColumns are the columns migration 14 adds.
var telegramColumns = []string{
	"telegram_bot_token_encrypted",
	"telegram_chat_id",
}

// migrationV14 returns the registered version 14 migration.
func migrationV14(t *testing.T) Migration {
	t.Helper()
	...
	t.Fatal("migration version 14 is not registered")
	return Migration{}
}

// TestMigrationV14_Registered verifies the migration is wired into the
// schema manager and will be reached by an upgrade. LatestVersion is
// asserted to be at least 14 rather than exactly 14 because later
// migrations are expected to follow.
func TestMigrationV14_Registered(t *testing.T) {
	sm := NewSchemaManager()
	if got := sm.LatestVersion(); got < 14 {
		t.Errorf("LatestVersion() = %d, want at least 14", got)
	}
	if desc := migrationV14(t).Description; desc == "" {
		t.Error("migration 14 has an empty description")
	}
}
...
func assertChannelTypeCheck(t *testing.T, pool *pgxpool.Pool) {
	...
	for _, channelType := range accepted {
		_, err := pool.Exec(ctx, `
			...
			VALUES ($1, $2, 'v14-test-owner')
		`, channelType, "v14-"+channelType)
		if err != nil {
	...
	_, err := pool.Exec(ctx, `
		INSERT INTO notification_channels (channel_type, name, owner_username)
		VALUES ('carrier_pigeon', 'v14-invalid', 'v14-test-owner')
	`)
	if err == nil {
	...
	if _, err := pool.Exec(ctx, `
		DELETE FROM notification_channels WHERE owner_username = 'v14-test-owner'
	`); err != nil {
		t.Fatalf("failed to clean up seeded channels: %v", err)
	}
}

// TestMigrationV14_FreshInstall verifies that a freshly migrated
// datastore carries both columns and the widened constraint.
func TestMigrationV14_FreshInstall(t *testing.T) {
...
// TestMigrationV14_UpgradesLegacyInstall rewinds a migrated datastore to
...
func TestMigrationV14_UpgradesLegacyInstall(t *testing.T) {
	ctx := context.Background()
	pool, conn := getTestConnection(t)
	if pool == nil {
		return
	...
	_, err = pool.Exec(ctx, `
		INSERT INTO notification_channels (channel_type, name, owner_username)
		VALUES ('slack', 'v14-legacy', 'v14-legacy-owner')
	`)
	if err != nil {
	...
	_, err = pool.Exec(ctx, `
		INSERT INTO notification_channels (channel_type, name, owner_username)
		VALUES ('telegram', 'v14-too-early', 'v14-legacy-owner')
	`)
	if err == nil {
	...
	if err := migrationV14(t).Up(tx); err != nil {
		t.Fatalf("migration 14 failed on a legacy install: %v", err)
	}
	if err := tx.Commit(ctx); err != nil {
		t.Fatalf("failed to commit migration 14: %v", err)
	}
	...
	err = pool.QueryRow(ctx, `
		...
		WHERE owner_username = 'v14-legacy-owner'
	`).Scan(&surviving)
	if err != nil {
	...
	if _, err := pool.Exec(ctx, `
		DELETE FROM notification_channels WHERE owner_username = 'v14-legacy-owner'
	`); err != nil {
...
// TestMigrationV14_IsIdempotent re-runs the migration over an install
// that already has it applied. Every statement is guarded, so a second
// run must be a no-op rather than an error.
func TestMigrationV14_IsIdempotent(t *testing.T) {
	ctx := context.Background()
	pool, conn := getTestConnection(t)
	if pool == nil {
		return
	...
	for i := 0; i < 2; i++ {
		...
		if err := migrationV14(t).Up(tx); err != nil {
			...
			t.Fatalf("re-run %d of migration 14 failed: %v", i+1, err)
		}
		if err := tx.Commit(ctx); err != nil {
...
// TestMigrationV14_ReportsStatementFailure drives the migration's error
...
func TestMigrationV14_ReportsStatementFailure(t *testing.T) {
	ctx := context.Background()
	pool, conn := getTestConnection(t)
	if pool == nil {
		return
	...
	err = migrationV14(t).Up(tx)
	if err == nil {
		t.Fatal("migration 14 succeeded without a notification_channels table")
	}
	if !strings.Contains(err.Error(), "Telegram notification channel support") {
		t.Errorf("migration 14 error = %q, want it to name the failure", err.Error())
	}
}
