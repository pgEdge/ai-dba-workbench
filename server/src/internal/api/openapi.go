	...
	return map[string]*OpenAPISchema{
		...
		"NotificationChannelCreateRequest": {
			...
			Properties: map[string]*OpenAPISchema{
				...
				"telegram_bot_token":      {Type: "string", Description: "Telegram bot token, in the form <bot id>:<token> (telegram). Required on POST for telegram channels. On PUT: omit to keep existing, empty string to clear, value to replace. Never returned; the response exposes telegram_bot_token_set instead."},
				"telegram_chat_id":        {Type: "string", Description: "Telegram chat ID or @channelusername (telegram). Required on POST for telegram channels. On PUT: omit to keep existing, empty string to clear, value to replace."},
				"reminder_enabled":        {Type: "boolean", Description: "Whether reminders are enabled"},
				"reminder_interval_hours": {Type: "integer", Description: "Reminder interval in hours"},
			},
		},
