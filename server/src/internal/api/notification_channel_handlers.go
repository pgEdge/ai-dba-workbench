...
func (h *NotificationChannelHandler) updateChannel(w http.ResponseWriter, r *http.Request, id int64) {
	...
	// channel without a token cannot deliver anything. A token that
	// comes from storage is checked for presence but not for shape; see
	// validateTelegramFields for why re-checking it would make a
	// channel with an undecryptable token impossible to edit or even to
	// disable.
	if channelType == string(database.ChannelTypeTelegram) {
		botToken := existing.TelegramBotToken
		botTokenFromRequest := req.TelegramBotToken != nil
		if botTokenFromRequest {
		...
		if msg := validateTelegramFields(botToken, chatID, botTokenFromRequest); msg != "" {
			RespondError(w, http.StatusBadRequest, msg)
			return
		}
	}
...
//
// botTokenFromRequest says which of those two the token is. A token the
// request supplied is shape-checked; a token that came from storage is
// only checked for presence. That asymmetry is deliberate.
// decryptNotificationSecret returns the raw stored value when
// decryption fails - which is what happens when the server secret has
// been rotated or lost - and that base64 ciphertext can never match
...
// channel off. Checking only what the request supplies loses nothing:
// that is the only moment at which the check can keep a bad value out
// of the database.
//
// No message includes the submitted value: the bot token is a bearer
// credential and error responses are quotable.
func validateTelegramFields(botToken, chatID *string, botTokenFromRequest bool) string {
	if botToken == nil || *botToken == "" {
		return "telegram_bot_token is required for telegram channels"
	}
	if botTokenFromRequest && !telegramBotTokenPattern.MatchString(*botToken) {
