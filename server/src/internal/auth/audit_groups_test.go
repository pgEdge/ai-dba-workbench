...
// TestScanStringsTxScanFailure covers the scan branch of the shared
// single-column helper. A NULL cannot be scanned into a string, which
// is what a column the caller believed to be NOT NULL would produce if
// the schema ever drifted.
func TestScanStringsTxScanFailure(t *testing.T) {
	store, cleanup := createTestAuthStoreForAudit(t)
	defer cleanup()

	tx := mustBeginTx(t, store)

	_, err := scanStringsTx(tx, "SELECT NULL", "failed to list things")
	if err == nil {
		t.Fatal("Expected a scan error for a NULL value")
	}
	if !strings.Contains(err.Error(), "failed to list things") {
		t.Errorf("Expected the caller's failure text, got %v", err)
	}
}

// TestGroupConnectionPrivilegesTxScanFailure covers the scan branch of
// the privilege snapshot read, by presenting the helper with a NULL
// access level in place of the text the column is declared to hold.
func TestGroupConnectionPrivilegesTxScanFailure(t *testing.T) {
	store, cleanup := createTestAuthStoreForAudit(t)
	defer cleanup()

	groupID := mustCreateAuditGroup(t, store, "dbas", "Database administrators")
	if err := store.GrantConnectionPrivilege(groupID, 1, "read"); err != nil {
		t.Fatalf("GrantConnectionPrivilege failed: %v", err)
	}

	// Swap the table for a view of the same shape whose access level is
	// always NULL.
	mustExec(t, store,
		"ALTER TABLE connection_privileges RENAME TO connection_privileges_real")
	mustExec(t, store, `CREATE VIEW connection_privileges AS
        SELECT group_id, connection_id, NULL AS access_level
        FROM connection_privileges_real`)

	tx := mustBeginTx(t, store)

	_, err := groupConnectionPrivilegesTx(tx, groupID)
	if err == nil {
		t.Fatal("Expected a scan error for a NULL access level")
	}
	if !strings.Contains(err.Error(),
		"failed to list group connection privileges") {
		t.Errorf("Expected the privilege failure text, got %v", err)
	}
}
