...
func (ps *ProbeScheduler) scheduleProbe(ctx context.Context, probe MetricsProbe) {
	defer ps.wg.Done()
...
func (ps *ProbeScheduler) executeProbeForConnection(ctx context.Context, probe MetricsProbe, conn MonitoredConnection) {
	if probe.IsDatabaseScoped() {
		...
		ps.executeProbeForAllDatabases(ctx, probe, conn)
	} else {
		// Execute probe once for the connection
		ps.executeProbeForSingleDatabase(ctx, probe, conn, "")
	}
}
