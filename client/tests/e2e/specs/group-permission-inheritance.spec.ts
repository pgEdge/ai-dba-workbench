...
test.describe('Group Permission Inheritance', () => {
    ...
    test('group privileges propagate to members and are revoked on removal', async ({ page }) => {
        ...
        await test.step('Phase 2: verify user can create a user via API (manage_users)', async () => {
            ...
            const { users } = await api.listUsers(testUserCookie);
            const probeUser = users.find((u) => u.username === probeUsername);
            if (probeUser) {
                await api.deleteUser(adminCookie, probeUser.id);
            }
        });
        ...
        await test.step('Phase 5: remove user from group via API', async () => {
            await api.removeGroupMember(adminCookie, groupId, userId);
        });
        ...
        if (aiEnabled) {
            await test.step('Phase 6 (AI): all Ellie prompts denied after revocation', async () => {
                // Need to select a server first — but tree is empty,
                // so the chat FAB will not render. Verify by checking
                // that the FAB is not present.
                await expect(
