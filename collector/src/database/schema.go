...
func (sm *SchemaManager) registerMigrations() {
	...
	sm.migrations = append(sm.migrations, Migration{
		...
		`)
			...
			COMMENT ON INDEX idx_user_accounts_username IS
				'Index for fast lookup of users by username';
			COMMENT ON INDEX idx_user_accounts_email IS
		...
		`)
			...
			CREATE INDEX IF NOT EXISTS idx_service_tokens_name ON service_tokens(name);
			...
			COMMENT ON INDEX idx_service_tokens_name IS
				'Index for fast lookup of service tokens by name';
			COMMENT ON INDEX idx_service_tokens_token_hash IS
				'Index for fast authentication by token hash';
			COMMENT ON INDEX idx_service_tokens_expires_at IS
		...
		`)
			...
			COMMENT ON INDEX idx_user_tokens_user_id IS
				'Index for fast lookup of tokens by user (foreign key index)';
			COMMENT ON INDEX idx_user_tokens_token_hash IS
				'Index for fast authentication by token hash';
			COMMENT ON INDEX idx_user_tokens_expires_at IS
				'Index for efficiently finding and cleaning up expired tokens';
