...
import { contrastRatio, WCAG_AA_NORMAL_TEXT } from '../../../test/contrast';
...
import AdminAuditLog, { MAX_ERROR_MESSAGE_LENGTH } from '../AdminAuditLog';
...
const okResponse = (events: AuditEvent[], total?: string | null): Response =>
    ({
        ok: true,
        status: 200,
        headers: {
            get: (name: string) =>
                name.toLowerCase() === 'x-total-count' ? (total ?? null) : null,
        },
...
const lastParams = (): URLSearchParams => new URLSearchParams(lastUrl().split('?')[1] ?? '');
...
/** Parse a `#rrggbb` colour into RGB channels. */
const hexRgb = (colour: string): [number, number, number] => {
    const parts = /^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i.exec(colour);
    if (!parts) {
        throw new Error(`Not a hex colour: ${colour}`);
    }
    return [parts[1], parts[2], parts[3]].map((v) => Number.parseInt(v, 16)) as [
        number,
        number,
        number,
    ];
...
/**
 * Parse an `rgb()` or `rgba()` string into RGB channels, compositing a
 * translucent colour over the given paper colour so the result is what
 * the chip label is actually read against.
 */
const paintedRgb = (colour: string, paper: [number, number, number]): [number, number, number] => {
    const parts = /rgba?\(([^)]+)\)/.exec(colour);
    if (!parts) {
        throw new Error(`Not an rgb colour: ${colour}`);
    }
    const [r, g, b, a = 1] = parts[1].split(',').map((v) => Number.parseFloat(v.trim()));
    return [r, g, b].map((channel, i) => channel * a + paper[i] * (1 - a)) as [
        number,
        number,
        number,
    ];
...
describe('AdminAuditLog', () => {
    ...
    // The outcome chips are painted by the pgEdge theme's MuiChip
    // overrides, so the contrast is measured under that theme in both
    // modes rather than under the transition-free test theme, whose
    // stock MUI chip colours say nothing about what users see.
    describe.each(['light', 'dark'] as const)('under the pgEdge %s theme', (mode) => {
        const theme = createPgedgeTheme(mode);
        const paper = hexRgb(theme.palette.background.paper);

        it.each<[AuditEvent['outcome'], AuditEvent[]]>([
            ['success', EVENTS],
            ['denied', EVENTS],
            ['failure', [SPARSE_EVENT]],
        ])('paints the %s outcome chip with WCAG AA contrast', async (outcome, events) => {
            mockApiFetch.mockResolvedValue(okResponse(events, '1'));
            render(
            ...
            const label = await screen.findByText(outcome);
            const chip = label.closest('.MuiChip-root');
            expect(chip).not.toBeNull();

            const chipStyle = window.getComputedStyle(chip as Element);
            const ratio = contrastRatio(
                paintedRgb(chipStyle.color, paper),
                paintedRgb(chipStyle.backgroundColor, paper)
            );
    ...
    it('sends the filter values when Apply is clicked', async () => {
        ...
        expect(params.get('since')).toBe(new Date('2026-09-01T00:00').toISOString());
        expect(params.get('until')).toBe(new Date('2026-09-15T12:00').toISOString());
    });
    ...
    it('reveals the formatted details and error text when expanded', async () => {
        ...
        expect(details.textContent).toContain(JSON.stringify(EVENTS[0].details, null, 2));

        fireEvent.click(expanders[1]);
        const denied = await screen.findByTestId('audit-details-11');
        expect(denied.textContent).toContain('requires manage_groups permission');
    });
    ...
    it('shows the actor IP as a tooltip on the actor name', async () => {
        ...
        expect(await screen.findByRole('tooltip')).toHaveTextContent('192.0.2.5');
    });
    ...
    it('renders the server error message when the request fails', async () => {
        mockApiFetch.mockResolvedValue(errorResponse(403, '{"error":"superuser access required"}'));
        renderWithTheme(<AdminAuditLog />);

        expect(await screen.findByText('superuser access required')).toBeInTheDocument();
    });
    ...
    it('falls back sensibly for a sparse event', async () => {
        ...
        expect(details.textContent).toContain('No further detail was recorded.');
    });
    ...
    it('renders a non-JSON error body verbatim', async () => {
        mockApiFetch.mockResolvedValue(errorResponse(500, 'upstream failure'));
        renderWithTheme(<AdminAuditLog />);

        expect(await screen.findByText('upstream failure')).toBeInTheDocument();
    });

    it('caps an oversized error body before showing it', async () => {
        const page = `<html>${'x'.repeat(5000)}</html>`;
        mockApiFetch.mockResolvedValue(errorResponse(502, page));
        renderWithTheme(<AdminAuditLog />);

        const banner = await screen.findByRole('alert');
        const shown = banner.textContent ?? '';
        expect(shown.length).toBeLessThanOrEqual(MAX_ERROR_MESSAGE_LENGTH + 1);
        expect(shown.endsWith('\u2026')).toBe(true);
        expect(shown.startsWith('<html>xxx')).toBe(true);
    });

    it('caps an oversized JSON error field before showing it', async () => {
        const long = 'y'.repeat(MAX_ERROR_MESSAGE_LENGTH + 50);
        mockApiFetch.mockResolvedValue(errorResponse(500, JSON.stringify({ error: long })));
        renderWithTheme(<AdminAuditLog />);

        const banner = await screen.findByRole('alert');
        expect(banner.textContent).toBe(`${'y'.repeat(MAX_ERROR_MESSAGE_LENGTH)}\u2026`);
    });
    ...
    it('reports the status when the error body is empty', async () => {
        ...
        expect(await screen.findByText('Request failed with status 502')).toBeInTheDocument();
    });
