    ...
    mockUseMetrics.mockImplementation((params) => {
        ...
        switch (key) {
            ...
            case 'blks_hit,blks_read':
                return ready([
                    series('blks_hit', [900, 950]),
                    series('blks_read', [100, 50]),
                ]);
            case 'temp_bytes':
    ...
    describe('KPI tiles', () => {
        ...
        it('requests the per-second block metrics for the cache KPI only', async () => {
            ...
            cacheCalls.forEach(p => {
                ...
                expect(p?.buckets).toBe(30);
                expect(p?.aggregation).toBe('avg');
            });
            // The Block I/O chart still plots the raw counters (issue #400).
            expect(seriesValuesFor('Block I/O', 'Blocks Hit')).toBe('900,950');
        });
