...
altering any event invalidates every hash that follows. Each event also
records, in its `hash_version` field, the version of the encoding its
hash was computed under, and verification recomputes every event under
the version it carries, so a future change to the format leaves earlier
events verifiable and an event claiming a version the server does not
know is reported as such rather than as a broken hash. The encoding
records the length of each field alongside its value, so moving text
...
updates to the `audit_events` table outright, and the only deletions the
server issues come from the retention purge described below.
...
an attempt to cover them up, which is what a mistaken `DELETE`, a
careless cleanup script or a misdirected restore looks like, and they
raise the number of deliberate steps required to hide a deletion from
one to several. They are not a defence against an attacker with write
access to `auth.db`.

The hash chain is an unkeyed SHA-256, computed by the same server that
stores it, so anyone who can write the file can recompute every hash
after making a change and leave a chain that verifies cleanly. The
record of the highest identifier issued is an ordinary table that the
same party can set to whatever value makes the comparison agree. An
offline verification that passes is therefore evidence that nothing
